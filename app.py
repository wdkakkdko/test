...
class Database_Manager:
    ...
    def init_item_database(self):
        self.cursor.execute("CREATE TABLE IF NOT EXISTS item(id INTEGER PRIMARY KEY AUTOINCREMENT," \
                                                    "name TEXT," \
                                                    "position TEXT,"\
                                                    "tag TEXT)") # テーブル：itemeを生成(id,name,position,tag1,tag2,tag3をカラムとする)
...
class Application:
    ...
    def gemini_generate_cont(self,value,group):
        value = list(value)
        print(value)
        self.res = self.model.generate_content(f"リストで与えられた{value}を各要素ごとに一般的に用いられている固有名詞でグループ分けし、{value}の各要素ごとに付加された3つのグループ名を出力してください。現在{group}のグループがあります。" \
                                                "既存のグループに組み分けられる場合は既存のグループを用い、組み分けれない場合は新たにグループを作成してください。" \
                                                "このときグループ名から商品の種別が連想できるように、実在する名称で命名してください。" \
                                                "ただし、回答はグループ名のみを出力してください。" \
                                                "例として、[かっぱえびせん,うまい棒]が与えられた際、出力はお菓子　スナック　食べもの,お菓子　スナック　食べものです。"\
                                                "また、[ハーゲンダッツ,あまおう]が与えられた場合、出力はアイスクリーム　デザート　食べもの,いちご　果物　食べものです"\
                                                "なお、「商品名」という単語が含まれている列は無視すること、数字は無視し、文字列だけを抜き出すこと、改行された単語、およびカンマで区切られた単語は異なる名詞であるとみなし、それぞれ各単語にあった適切なワードで名詞ごとにグループ分けすること、グループ名に記号を用いないことを守ってください")
    ...
    def csv_button_clicked(self,event):
        ...
        res_list = self.gemini_generate_cont(pd_csv['name'],group)
